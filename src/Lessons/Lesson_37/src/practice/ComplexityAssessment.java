...
public class ComplexityAssessment {
    ...
    // Метод 8
    /**
     * Выполняет два цикла. Первый цикл выполняет вывод значений от 0 до {@code n-1}.
     * Второй цикл состоит из двух вложенных циклов, которые выполняют вывод произведения индексов {@code i * j}.
     *
     * <p>Анализ временной сложности:</p>
     *
     * <ul>
     *   <li><b>Первый цикл:</b> {@code for (int i = 0; i < n; i++)} — выполняется {@code n} раз, и на каждой итерации выводится {@code i}. Это операция выполняется за {@code O(1)}, следовательно, временная сложность для первого цикла — {@code O(n)}.</li>
     *
     *   <li><b>Второй цикл:</b> состоит из двух вложенных циклов:
     *     <ul>
     *       <li><b>Внешний цикл:</b> {@code for (int i = 0; i < n; i++)} — выполняется {@code n} раз.</li>
     *       <li><b>Внутренний цикл:</b> {@code for (int j = 0; j < n; j++)} — выполняется {@code n} раз для каждой итерации внешнего цикла.</li>
     *     </ul>
     *     Таким образом, два вложенных цикла выполняются {@code n * n = n²} раз, и на каждой итерации выполняется операция вывода {@code System.out.println(i * j)}, что занимает {@code O(1)}.</li>
     * </ul>
     *
     * <p><b>Общая временная сложность:</b></p>
     * <pre>
     *     O(n) (первый цикл) + O(n²) (второй цикл) = O(n + n²)
     * </pre>
     *
     * <p>Поскольку при асимптотическом анализе учитываются только наибольшие порядки, итоговая временная сложность метода — {@code O(n²)}.</p>
     *
     * @param n Определяет количество итераций для первого и второго цикла.
     */
